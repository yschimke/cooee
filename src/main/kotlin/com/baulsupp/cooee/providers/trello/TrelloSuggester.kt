...
val TrelloHomepage = Suggestion("trello", description = "Trello Homepage", url="https://trello.com", type=SuggestionType.LINK)
...
class TrelloSuggester(val provider: TrelloProvider) : Suggester {
  ...
  private fun boardCompletion(it: BoardResponse) =
    Suggestion(it.url.split("/").last(), description = "Trello Board: ${it.name}", url=it.url, type=SuggestionType.LINK)
}
