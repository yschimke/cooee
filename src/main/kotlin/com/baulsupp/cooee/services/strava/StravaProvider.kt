...
class StravaProvider : Provider("strava", StravaAuthInterceptor().serviceDefinition) {
  override suspend fun runCommand(request: CommandRequest): Flow<CommandResponse>? = when {
    request.parsed_command == listOf("strava") -> flowOf(stravaWebsite)
    request.parsed_command.getOrNull(1) == "last" -> flowOf(lastRun() ?: CommandResponse.error("No activities"))
    else -> null
  }
  ...
  override suspend fun suggest(command: CompletionRequest): List<CompletionSuggestion> {
    return listOf(
        CompletionSuggestion.command(
            CommandSuggestion(provider = "strava", description = "Strava Website"), "strava"),
        CompletionSuggestion.command(
            CommandSuggestion(provider = "strava", description = "Last Activity in Strava"),
            "strava", "last")
    )
  }
